...
CLN_ONION=`run-in-node c-lightning 'lightning-cli getinfo | jq -r ".address[].address"'`
CLN_REST=`run-in-node tor 'cat /var/lib/tor/hidden_service_cln_rest/hostname'`
CLN_MACAROON=`run-in-node c-lightning 'xxd -ps -u -c 1000 /lightningd/cln-plugins/c-lightning-REST-0.10.5/certs/access.macaroon'`
CLN_ID=`run-in-node c-lightning 'lightning-cli getinfo | jq -r ".id"'`
